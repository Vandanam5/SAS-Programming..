...
LIBNAME TELECOM "C:\Users\vanda\OneDrive\Desktop\Telecom company";
...
infile "C:\Users\vanda\OneDrive\Desktop\Telecom company\New_Wireless_Fixed.txt";
input acctno 1-14
@15 actdt mmddyy10.
@26 Deactdt mmddyy10.
Deactreason $ 41-45
...
proc sql;
select nmiss(Province) as Province,nmiss (deactreason) as deactreason,nmiss(rateplan) as rateplan,nmiss(dealertype) as dealertype
from telecom.Data;
quit;
...
*EXTRA QUESTIONS
*FINDING THE DISTRIBUTION OF PROVINCE,AGE GROUP AND SALES GROUP BASED ON ACCOUNT STATUS ;

*ACCOUNT STATUS BASED ON PROVINCE;
PROC SGPLOT DATA =telecom.TENURE_SEGNEW;
VBAR  PROVINCEE/ group =account_status groupdisplay = CLUSTER;
YAXIS LABEL = "ACCOUNT";
RUN;
*ACCOUNT STATUS BASED ON AGE GROUP;
PROC SGPLOT DATA =telecom.TENURE_SEGNEW;
VBAR  Age_group/ group =account_status groupdisplay = CLUSTER;
YAXIS LABEL = "ACCOUNT";
RUN;
*ACCOUNT STATUS BASED ON SALES GROUP;
PROC SGPLOT DATA =telecom.TENURE_SEGNEW;
VBAR  sales_group/ group =account_status groupdisplay = CLUSTER;
YAXIS LABEL = "ACCOUNT";
RUN;

*ACCOUNT STATUS BASED ON DEALER TYPE;

PROC SGPLOT DATA =telecom.TENURE_SEGNEW;
VBAR  DEALERTYPE/ group =account_status groupdisplay = CLUSTER;
YAXIS LABEL = "ACCOUNT";
RUN;
